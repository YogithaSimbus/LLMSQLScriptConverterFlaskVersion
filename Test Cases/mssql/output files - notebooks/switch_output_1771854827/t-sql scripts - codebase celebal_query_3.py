...
# MAGIC %md
# MAGIC # t-sql scripts - codebase celebal_query_3
# MAGIC This notebook was automatically converted from the script below. It may contain errors, so use it as a starting point and make necessary corrections.
# MAGIC
# MAGIC Source script: `/Volumes/lakebridge/switch/switch_volume/upload_1771854827/t-sql scripts - codebase celebal_query_3.sql`
...
SELECT 
    ...
    SUM(CAST(`Amount before Tax USD` AS DECIMAL(10,2))) AS Amount
FROM 
    MNS_Shell_Business_Mobility_Detailed
WHERE 
    (`Consumption Period` IN ('201912', '202012') 
     OR `Consumption Period` >= LEFT(CURRENT_DATE() - INTERVAL 6 MONTH, 6))
GROUP BY 
    `Consumption Period`, 
    `GIDUser Business`, 
